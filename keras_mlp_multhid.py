...
model.add(Dense(500,input_dim=ndim, W_regularizer=l2(0.01),init='normal')) # hidden layer dont want  good res :150,200(87), (inp:25,dim:250, 91%) 500 94 prev 
model.add(Activation('sigmoid'))
model.add(Dropout(0.001)) # acts as a regulalizer 0.09

model.add(Dense(250,init='normal', W_regularizer=l2(0.01))) # hidden layer dont want 200, 40 is output size, will stay same, gaussian inst of UNIFORM
model.add(Activation('sigmoid'))
model.add(Dropout(0.001)) # acts as a regulalizer 0.09

# try with more hid layers, obj can we beat 95???
...
history=model.fit(Xtraining,y_train,nb_epoch=2000,batch_size=10,verbose=0)        # epoch:100 batchsize: 20 useful to inc later  
